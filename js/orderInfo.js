...
$(function(){
    ...
    $(".btn-count-2").on("click",function(){
        ...
        if($("#consignee").val()==''){
            ...
            $("#consignee-error").addClass('error');
        }
        if($("#Id_card").val()==''){
            $("#Id_card").addClass('error');
            $("#Id_card-error").addClass('error');
        }
        if($("#address").val()==''){
        ...
        if($("#phone_mob").val()==''){
            $("#phone_mob").addClass('error');
            $("#phone_mob-error").addClass('error');
        }
        if((!$('select').hasClass('error')&&!$('input').hasClass('error')&&$("#consignee").val()!=""&&$("#Id_card").val()!=""&&$("#adress").val()!=""&&$("#phone_mob").val()!="")){
            $.cookie('pay-num',$(".sumprice span").text());
...
//表单验证
function validate(){
    validator = $('#address-form').validate({
        debug: true,//非真提交，仅作为验证
        rules:{
            name:{
                required:true,
                minlength:2,
        ...
        message:{
            ...
            address:{
                required:"请输入详细地址",
                maxlength:"详细地址不能超过15个字符",
                minlength:"详细地址不能少于5个字符"
            },
            tel:{
                required:"请输入手机号",
            }
    ...
    //    自定义添加身份证验证
    $.validator.addMethod("cardNo",function(value,element,params){
        var cardNo = /^\d{6}((?:19|20)(?:(?:\d{2}(?:0[13578]|1[02])(?:0[1-9]|[12]\d|3[01]))|(?:\d{2}(?:0[]13456789|1[012])(?:0[1-9]|[12]\d|30))|(?:\d{2}02(?:0[1-9]|1\d|2[0-8]))|(?:(?:0[48]|[2468][048]|[13579][26])0229)))\d{2}(\d)[xX\d]$/;
        return this.optional(element) || (cardNo.test(value));
    }, $.validator.format("请填写正确身份证号码!"));
    //自定义手机号验证
    $.validator.addMethod("tel",function(value,element,params){
        var tel =/^1[34578]\d{9}$/;
        return this.optional(element) || (tel.test(value));
    }, $.validator.format("请填写正确手机号码!"));
}
        ...
        success: function (res) {
            ...
            if (sc_str) {
                ...
                for (var i in sc_obj) {
                    ...
                    html+='<ul class="tr clearfix" data-good-id="278576" data-good-quantity="1" data-checkout-goods="">\
                            <li class="td item">\
                            <div class="img cover-img" style="background-image:url(' + res[sc_obj[i].id - 1].url + ')"; title=' + res[sc_obj[i].id - 1].title + res[sc_obj[i].id - 1].brank + res[sc_obj[i].id - 1].name+ '></div>\
                            <p>\
                                 <a href="/detail-278576.html" class="clearfix" target="_blank" data-good-id="278576">' + res[sc_obj[i].id - 1].brank + '</a>\
                                 <br>\
                                 <a href="/detail-278576.html" class="clearfix" target="_blank" data-good-id="278576">' + res[sc_obj[i].id - 1].name + '</a>\
                            </p>\
                        </li>\
                        <li class="td price">\
                            <p>' + res[sc_obj[i].id - 1].nowprice + ' </p>\
                        </li>\
                        <li class="td number">\
                            <p>x' + sc_obj[i].num + '</p>\
                        </li>\
                        <li class="td total">\
                            <p data-good-price="">' + res[sc_obj[i].id - 1].nowprice + '</p>\
                        </li>\
                        </ul>';
                }
                // console.log(html);
                $('#order').html(html);//商品信息 添加到 购物车详情页面
            }
        }
    })
}
